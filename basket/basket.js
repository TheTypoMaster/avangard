	...
	req.onreadystatechange = function () {
		if (req.readyState==4) {
			if (req.status==200) {
				...
				if (regText) document.getElementById('basket_text').innerHTML="&nbsp; <a href='/basket' style=\"color:#78A40E\">Товаров в корзине - "+regText+"</a>"; //return value
			}
		}
	};
	req.open("GET", "/basket/basket_ajax.php?count=1");
...
function addSubject(subj_id, combinac_id, recommend_id) {
	...
	req.onreadystatechange = function () {
		if (req.readyState==4) {
			if (req.status==200 && callback) {
				...
				var regText = req.responseText;
				//alert('regTextMetro - '+regText);
				document.getElementById('basket_text').innerHTML="&nbsp; <a href='/basket' style=\"color:#78A40E\">Товаров в корзине - "+regText+"</a>"; //return value
	...
	req.open("GET", "/basket/basket_ajax.php?subj_id="+subj_id+'&combinac_id='+combinac_id+'&recommend_id='+recommend_id);
	req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;charset=utf-8"); // set Header
...
function delSubject(key) {
	...
	req.open("GET", "/basket/basket_ajax.php?del_key="+key);
	req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;charset=utf-8"); // set Header
	req.send(null); //send value
	//return false;
} 
...
function delAllSubject() {
	...
	req.open("GET", "/basket/basket_ajax?del_all=1");
	req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;charset=utf-8"); // set Header
	req.send(null); //send value
	return false;
} 
