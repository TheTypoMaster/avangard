...
##### проба		
$db_props = CIBlockElement::GetProperty(31, 
                                        $subject[subj_id], "sort", 
                                        "asc", 
                                        Array("CODE"=>"FORUM_TOPIC_ID"));
$ar_props = $db_props->Fetch();
echo "<pre>$ar_props:"; print_r($ar_props); echo "</pre>";
##### проба	
		...
		if ($subject[recommend_id] > 0 && $arCombinac[ID] && !isset($_GET[rec_color])) { // выбираем рекомендуемые цвета  |||   && ($_GET[rec_color]<>0 && isset($_GET[rec_color]) )
			...
			while ($ob = $res->GetNextElement()) {
				...
				if ($arFields[PREVIEW_PICTURE]) {
					...
					$arFileSubj = $resFileSubj->Fetch();
###### пока закомментим						$srcSubj = "/upload/" . $arFileSubj[SUBDIR] . "/" . $arFileSubj[FILE_NAME];
				}
...
//				echo "$key_in_basket srcTexture12 - ";				print_r($srcTexture);
		...
		<tr id="tr_<?= $key_in_basket ?>">
			<td valign="middle"><a href="/wharetobuy/shop/element.php?id=<?= $arSubj[ID] ?>"><?= $arSubj[NAME] ?></a></td>
