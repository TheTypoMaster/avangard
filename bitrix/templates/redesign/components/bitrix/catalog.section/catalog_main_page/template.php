...
<table width="738" align="center">
	...
	if ($arResult['CAT_ARRAY'][7468])  $arResult['CAT_ARRAY'][7468][name] = '—ветильники и торшеры';
	$kol = 0;
	foreach($arResult['CAT_ARRAY'] as $category){
		...
		foreach($arResult['ITEMS'] as $el){
			if($el["DISPLAY_PROPERTIES"]["COLLECTION"]["VALUE"]!=$category["id"])//ѕропускаем если это не текуща€ коллекци€
				continue;
			if ($el["ID"] == 7781) 
				continue; // пропускаем нано наматрасник
			$i++;
			$rows_count--;
			if($i == 1)
		...
		if($el["DISPLAY_PROPERTIES"]["SKIDKA"]["VALUE"]) {
			$skidka_value = $el["DISPLAY_PROPERTIES"]["SKIDKA"]["VALUE"]; /* если есть скидка, то выводитс€ введенное значение */
