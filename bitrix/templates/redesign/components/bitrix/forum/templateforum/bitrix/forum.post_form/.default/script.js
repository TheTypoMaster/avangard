...
function PostForm()
{
	...
	this.Init = function(form)
	{
		...
		for (var res in this.tags)
		{
			if (this.form[res])
			{
				if (res == "FONT" || res == "COLOR")
					this.form[res].onchange = new Function("oForumForm['" + this.form.id + "']." + this.tags[res] + "(this);");
				else if (res == "CLOSE")
				{
					this.form[res].onclick = new Function("oForumForm['" + this.form.id + "']." + this.tags[res] + "(this);");
					this.form[res].onmouseover = function(){this.style.textDecoration = "underline"; oForumForm[this.form.id].show_hints(this.name)};
					this.form[res].onmouseout = function(){this.style.textDecoration = "none";};
				}
				else
				{
					this.form[res].onclick = new Function("oForumForm['" + this.form.id + "']." + this.tags[res] + "(this);");
					this.form[res].onmouseover = function(){oForumForm[this.form.id].show_hints(this.name)};
				}
				if (this.form[res].accessKey)
				{
					...
					this.nav = (userAgent.indexOf('safari')!=-1 ? 'Safari' : 'none');
		...
		if (image && (typeof(image) == "object" || typeof(image) == "function") && image.length > 0)
		{
			for (var ii = 0; ii < image.length; ii++ )
			{
				if (image[ii].className != "smiles")
				{
					image[ii].onclick =  new Function("oForumForm['" + this.form.id + "'].emoticon(this);");
				}
			}
		}
		if (this.form["POST_MESSAGE"])
		{
			this.form["POST_MESSAGE"].onselect = function(){oForumForm[this.form.id].store_caret(this)};
			this.form["POST_MESSAGE"].onclick = function(){oForumForm[this.form.id].store_caret(this)};
			this.form["POST_MESSAGE"].onkeyup = function(){oForumForm[this.form.id].store_caret(this)};
		}
		return true;
	}, 
	...
	this.simple_tag = function(element)
	{
		...
		if ((this.open[tag_name] == 0) && this.Insert("[" + tag_start + "]", "[/" + tag_name + "]", true))
		{
			this.open[tag_name]++;
			element.value += "*";
			this.stack.push(tag_name);
			this.show_hints('CLOSE_CLICK')
		}
		else
		{
	...
	{
		this.form['tagcount'].value = this.stack.length;
	},
		...
		if (selection == "")
		{
			...
			if (parseInt(message) > 0)
			{
				...
				if (typeof(message) == "object" && message)
				{
					...
					selection = selection.replace(/\<script[^\>]*>/gi, '\001');
					selection = selection.replace(/\<\/script[^\>]*>/gi, '\002');
					selection = selection.replace(/\<noscript[^\>]*>/gi, '\003');
					selection = selection.replace(/\<\/noscript[^\>]*>/gi, '\004');
					
					selection = selection.replace(/\001([^\002]*)\002/gi, " ");
