   ...
   <?        
	...
	while($ob = $res->GetNextElement())
	  ...
	  foreach($cat_array as $category) 
	   ...
	   $arElementFilter = Array("IBLOCK_ID"=>IntVal(5), "SECTION_ID"=>IntVal(2), "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "!PROPERTY_SLIDER"=>false,  "PROPERTY_COLLECTION"=>IntVal($category[id]));
 ...
 <tr><td colspan="5"><b><?=$category[name]?></b>
	   ...
	   while($obElement = $resElement->GetNextElement())
		...
		if($arElementFields['PROPERTY_BLACKWHITE_PIC_VALUE']) $img_path_bl = CFile::GetPath($arElementFields['PROPERTY_BLACKWHITE_PIC_VALUE']); else $img_path_bl = $img_path;
				...
				<td class="catalog_td">
				<a href="/catalog/divan<?=$arElementFields[ID]?>.htm"><img onMouseOver="this.src='<?=$img_path?>';" onMouseOut="this.src='<?=$img_path_bl?>';" class="catalog_picture" src="<?=$img_path_bl?>" alt="Модульная система <?=$arElementFields[NAME]?>"></a><br>
				<a class="catalog_name" href="/catalog/divan<?=$arElementFields[ID]?>.htm">Модульная система <?=$arElementFields[NAME]?></a>
