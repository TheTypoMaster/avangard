   ...
   <?        
	...
	while($ob = $res->GetNextElement())
	  ...
	  foreach($cat_array as $category) 
	   ...
	   $arElementFilter = Array("IBLOCK_ID"=>IntVal(5), "SECTION_ID"=>IntVal(73), "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "!PROPERTY_SLIDER"=>false,  "PROPERTY_COLLECTION"=>IntVal($category[id]));
	   ...
	   while($obElement = $resElement->GetNextElement())
		...
		if($arElementFields['PROPERTY_BLACKWHITE_PIC_VALUE']) $img_path_bl = CFile::GetPath($arElementFields['PROPERTY_BLACKWHITE_PIC_VALUE']); else $img_path_bl = $img_path;
				...
				<a class="catalog_name" href="/catalog/divan<?=$arElementFields[ID]?>.htm">Кровать <?=$arElementFields[NAME]?></a>
