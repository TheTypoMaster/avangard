...
############################### модификация   #########################################################################################
						...
						$arIDOrderElement = array();
						$i = 1;
						foreach ($APPLICATION->get_cookie("basket") as $key => $val){
							$subject = unserialize($val);
							$PROP = $arCombinac = $arRecomend = array();
							$PROP[SUBJECT] = $subject[subj_id];
							$PROP[COMBINATION] = $subject[combinac_id];
							$PROP[RECOMMEND] = $subject[recommend_id];
							$PROP[CHOOSE_MAT] = $subject[texture_id];
							
							$resSubj = CIBlockElement::GetByID($subject[subj_id]);
							$arSubj = $resSubj->Fetch();
							if ($subject[combinac_id]) {
								$resCombinac = CIBlockElement::GetByID($subject[combinac_id]);
								$arCombinac = $resCombinac->Fetch();
							}
							if ($subject[recommend_id]) {
								$resRecomend = CIBlockElement::GetByID($subject[recommend_id]);
								$arRecomend = $resRecomend->Fetch();
								/* получаем текстуры */
								$arTexture = $arTextureDesc = $arAllTexture= array();
								$res_text = CIBlockElement::GetProperty(21, $subject[recommend_id], "sort", "asc", array("CODE" => "TEXTURE"));
								while ($ob_text = $res_text->GetNext())
								{
									$resTexture = CIBlockElement::GetByID($ob_text['VALUE']);
									$arTexture = $resTexture->Fetch();
									$arAllTexture[] = $arTexture;
									$arTextureDesc[] = '<a href="http://'.$_SERVER['SERVER_NAME'].'/catalog/texture/'.$arTexture[ID].'.html">'.$arTexture[NAME].'</a>';
								}
								/***********************/
								
								/* цена */
								$resPrice = CIBlockElement::GetProperty(21, $subject[recommend_id], array("sort" => "asc"), Array("CODE"=>"PRICE"));
							...
							$arIdOrderElement[] = $PRODUCT_ID;
							$description = "";
							if (sizeof($arCombinac)) {
								$description .= ", комбинация - ".$arCombinac[NAME]."";
							}
							if (sizeof($arRecomend)) {
								if (sizeof($arTextureDesc))
									$description .= ", рекомендуемые материалы - (".implode(" | ", $arTextureDesc).")";
								if ($arPrice[VALUE])
									$description .= ", цена - ".$arPrice[VALUE]." руб.";
							}
							$arNameSubject[] = $i.". <a href=\"http://".$_SERVER['SERVER_NAME']."/catalog/subject/".$arSubj[ID].".html\">".$arSubj[NAME]."</a>".$description;
							
							$productPhoto = CFile::GetPath($arSubj["PREVIEW_PICTURE"]);
							$arProductsPhoto[] = $i.". <a href=\"http://".$_SERVER['SERVER_NAME']."/catalog/subject/".$arSubj[ID].".html\">".$arSubj[NAME]."</a><br /> <img src='http://".$_SERVER['SERVER_NAME'].$productPhoto."'/><br />";
							
							$APPLICATION->set_cookie("basket[".$key."]", "");
							$APPLICATION->set_cookie("basket[".$key."]", " ");
							mail ('quell0504@mail.ru', 'delete from basket', 'key - '.$key.'; ');
							$i++;
						}
						mail ('quell0504@mail.ru', 'need clear basket', 'APPLICATION->get_cookie("basket") - '.print_r($APPLICATION->get_cookie("basket"),1).'; ');
						CIBlockElement::SetPropertyValuesEx($arParams["ID"], false, array("SUBJECTS" => $arIdOrderElement)); 
						
						//Отправка письма менеджеру с фото дивана при оформлении заказа
						$arEventFields = array(
							"USER_NAME"            		=> $_REQUEST[PROPERTY][105][0],
							"USER_PHONE"            	=> $_REQUEST[PROPERTY][104][0],
							"USER_EMAIL"            	=> $_REQUEST[PROPERTY][113][0],
							"COMMENT"             		=> $_REQUEST[PROPERTY][106][0],
							"TEXT_ORDER"         		=> implode("<br>", $arProductsPhoto),
							"DATE"         				=> date("d.m.Y H:i"),  
							);
						CEvent::Send("neworder_photo", "rd", $arEventFields);
						...
						$arEventFields = array(
							...
							"USER_EMAIL"            	=> $_REQUEST[PROPERTY][113][0],
							"COMMENT"             		=> $_REQUEST[PROPERTY][106][0],
							"TEXT_ORDER"         		=> implode("<br>", $arNameSubject),
							"DATE"         				=> date("d.m.Y H:i"),  
							);
						CEvent::Send("order", "rd", $arEventFields);
						if (sizeof($APPLICATION->get_cookie("basket"))>0){ 
							foreach($APPLICATION->get_cookie("basket") as $del_key => $val){
								$APPLICATION->set_cookie("basket[".$del_key."]", "");
							}
						}
						mail ('quell0504@mail.ru', 'need clear basket2', 'APPLICATION->get_cookie("basket") - '.print_r($APPLICATION->get_cookie("basket"),1).'; _REQUEST[PROPERTY] - '.print_r($_REQUEST[PROPERTY],1).'; ');
						LocalRedirect('http://'/$_SERVER['SERVER_NAME'].'/shop/basket/');
						/*?>
