...
?>
					<li class="photo-control photo-control-first photo-control-photo-slideshow">
						<a rel="nofollow" href="<?=$arResult["SLIDE_SHOW"]?>" <?
							?> title="<?=GetMessage("P_SLIDE_SHOW_TITLE")?>">
							<span><?=GetMessage("P_SLIDE_SHOW")?></span></a>
					</li>
<?
	endif;
...
?>
					<li class="photo-control photo-control-photo-original">
						<a rel="nofollow" href="<?=$url?>" <?
							...
							?> onclick="ShowOriginal('<?=CUtil::JSEscape($url)?>', <?
									?>'<?=CUtil::JSEscape($arResult["ELEMENT"]["NAME"])?>'); return false;"><?
							?><span><?=GetMessage("P_ORIGINAL")?></span>
						</a>
					</li>
<?
	endif;
...
?>
					<li class="photo-control photo-control-first photo-control-photo-edit">
						<a href="<?=$arResult["ELEMENT"]["EDIT_URL"]?>" <?
							?> title="<?=GetMessage("P_EDIT_TITLE")?>" <?
							?> rel="nofollow" <?
							?> id="photo_edit" <?
							?> onclick="EditPhoto('<?=CUtil::JSEscape($arResult["ELEMENT"]["~EDIT_URL"])?>'); return false;">
							<span><?=GetMessage("P_EDIT")?></span></a>
					</li>
<?
	endif;
...
?>
					<li class="photo-control photo-control-photo-original">
						<a href="<?=$arResult["ELEMENT"]["DROP_URL"]?>" <?
							?> rel="nofollow" <?
							?> title="<?=GetMessage("P_DROP_TITLE")?>" <?
							?> onclick="return confirm('<?=GetMessage("P_DROP_CONFIM")?>');" <?
							?>id="photo_drop"><?
							?><span><?=GetMessage("P_DROP")?></span>
						</a>
					</li>
<?
	endif;
