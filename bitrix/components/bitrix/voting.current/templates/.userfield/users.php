...
if ((!empty($_REQUEST["ID"]) || !empty($_REQUEST["answer_id"])) && check_bitrix_sessid())
{
	...
	$cache = new CPHPCache();
	$cache_id = "vote_users_".serialize(array(
		$_REQUEST["ID"],
		$_REQUEST["answer_id"],
		$_REQUEST["iNumPage"],
	...
	$arUsers = (($arParams["CACHE_TIME"] > 0 && $cache->InitCache($arParams["CACHE_TIME"], $cache_id, $cache_path)) ?
		$cache->GetVars() : array());
	...
	if (!is_array($arUsers) || empty($arUsers))
	{
		if (empty($_REQUEST["ID"]) && CModule::IncludeModule("vote"))
		{
			...
			{
				$arVoteList["StatusPage"] = (($db_res->NavPageNome >= $db_res->NavPageCount ||
