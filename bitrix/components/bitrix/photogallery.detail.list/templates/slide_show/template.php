		...
		<table cellpadding="0" cellspacing="0" border="0" width="100%" class="photo-player-panel">
			<tr valign="middle">
				...
				<td align="center" width="20%">
					<table cellpadding="0" cellspacing="0" border="0" class="photo-player-panel-buttons">
						<tr>
							...
							<td><div class="photo-player-button" id="play" onclick="if(player){<?
								?>if (player.params['status'] == 'paused') {player.params['status']='play';player.play();} <?
								?>else {player.stop();}}"></div></td>
					...
					<table cellpadding="2" cellspacing="0" border="0" class="inner">
						<tr>
							<td><div class="photo-player-button" id="time_minus" onclick="if(player && player.params['period'] > 1){player.params['period']--;<?
								?>document.getElementById('time_container').innerHTML = player.params['period'] + '';}"></div></td>
				...
				<td width="10%" valign="top" align="right"><div id="stop" class="photo-player-button" <?
					...
					?>} <?
					if (!empty($arParams["BACK_URL"])):
						?>jsUtils.Redirect([], '<?=CUtil::JSUrlEscape($arParams["BACK_URL"])?>');<?
					endif;
					?>"></div></td>
			</tr>
		</table>
...
function to_init(e)
{
	...
	if (is_loaded)
	{
		...
		BPCSourse.prototype.OnBeforeSendData = function()
		{
			...
			{
				var res = document.getElementById(jj[ii]);
				if (res)
				{
		...
		{
			var res = document.getElementById('bx_slider_content_item_' + number);
			try
			{
			...
			document.getElementById('counter').innerHTML = item_id;
			document.getElementById('title').innerHTML = this.oSource.Data[item_id]['title'];
			return true;
		}
		...
		SlideSlider = new BPCSlider1(
			...
			'<?=CUtil::JSEscape(!empty($arParams["BACK_URL"]) ? $arParams["BACK_URL"] : $arParams["DETAIL_URL_FOR_JS"])?>' // url for redirect
			); 
		var windowSize = GetWindowSize();
		...
		else
		{
			...
			document.getElementById("bx_slider_content_item_1").innerHTML = '<div class="error">' + str.replace("#HREF#", window.location.href) + '</div>';
		}
	}
	else
	{
