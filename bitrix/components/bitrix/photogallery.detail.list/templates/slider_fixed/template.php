...
<table class="photo-navigation" cellpadding="0" cellspacing="0" border="0" style="height:<?=$panelHeight?>px;">
		...
		<td width="20px">
			<table class="photo-left" cellpadding="0" cellspacing="0" border="0" style="height:<?=$panelHeight?>px;">
				<tr style="height:<?=($panelHeight - 28)?>px;">
					<td	class="photo-left-top-disabled" id="photo_prev_<?=$package_id?>" onclick="if(window['player_slider']){window['player_slider'].step('prev');}">
						<div class="empty"></div></td></tr>
				<tr style="height:28px;">
					<td class="photo-left-bottom-disabled" id="photo_first_<?=$package_id?>" onclick="if(window['player_slider']){window['player_slider'].step('first');}">
						<div class="empty"></div></td></tr>
			</table>
		</td>
	...
	foreach ($arResult["ELEMENTS_CURR"] as $res):
		?><td id="td_<?=$i_cnt?>" style="width:<?=$cellWidth?>px;"><?
	...
	?></tr></tbody></table></td>
		<td width="20px">
			<table class="photo-right" cellpadding="0" cellspacing="0" border="0" style="height:<?=$panelHeight?>px;">
				<tr style="height:<?=($panelHeight - 28)?>px;">
					<td	class="photo-right-top-disabled" id="photo_next_<?=$package_id?>" onclick="if(window['player_slider']){window['player_slider'].step('next');}">
						<div class="empty"></div></td></tr>
				<tr style="height:28px;">
					<td class="photo-right-bottom-disabled" id="photo_last_<?=$package_id?>" onclick="if(window['player_slider']){window['player_slider'].step('last');}">
						<div class="empty"></div></td></tr>
			</table>
		</td>
	</tr>
...
{
	var is_loaded = false;
	try { is_loaded = (bPhotoMainLoad === true && bPhotoPlayerLoad === true && jsAjax && jsUtils); }
	catch(e){}
	
	if (is_loaded)
	{
		SliderCopy = new BPCSlider(
		...
		SliderCopy.ShowItem = function(item_id, number)
		{
			...
			document.getElementById('td_' + number).innerHTML = '<a href="' + res['url'] + '" ' +  <?
			if ($arParams["ELEMENT_ID"] > 0): 
				?>(res["id"] + '' == '<?=$arParams["ELEMENT_ID"]?>' ? ' class="active"' : '') + <?
			endif;?>
			'><img src="' + res['src'] + '" width="' + res['width'] + '" height="' + res['height'] + '" /></a>'; 
			
			var event_names = {'onclick' : '', 'onmouseover' : '', 'onmouseout' : '', 'onmousedown' : '', 'onmouseup' : ''};
			var arrID = {'photo_prev_<?=$package_id?>' : 'prev', 'photo_first_<?=$package_id?>' : 'prev', 'photo_last_<?=$package_id?>' : 'next', 'photo_next_<?=$package_id?>' : 'next'};
			for (var id in arrID)
			{
		...
		player_slider = new BPCPlayer(SliderCopy);
		player_slider.events['OnWaitItem'] = function(){if (!document.getElementById("wait_table_photo_photos_<?=$package_id?>_1")) {jsAjaxUtil.ShowLocalWaitWindow(1, 'table_photo_photos_<?=$package_id?>', false);} }
		player_slider.events['OnShowItem'] = function(){jsAjaxUtil.CloseLocalWaitWindow(1, 'table_photo_photos_<?=$package_id?>');}
	}
	else
	{
