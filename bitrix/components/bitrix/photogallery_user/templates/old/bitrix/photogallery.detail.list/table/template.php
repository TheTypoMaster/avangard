...
<?
	...
	if(!is_array($arItem["PICTURE"])):
		...
		$sImage = CFile::ShowImage($arItem["PICTURE"]["SRC"], $arParams["THUMBS_SIZE"], $arParams["THUMBS_SIZE"], "border=\"0\" vspace=\"0\" hspace=\"0\" alt=\"".htmlspecialchars($arItem["CODE"])."\"");
		$sTitle = $arItem["~NAME"].(!$bActiveElement ? GetMessage("P_PHOTO_NOT_APPROVED") : "");
		...
		elseif ($arParams["SHOW_CONTROLS"] == "Y" && $arParams["DetailListViewMode"] == "edit"):
			...
			?><a href="#" title="<?=htmlspecialchars($sTitle)?>" onclick="this.previousSibling.checked=!this.previousSibling.checked; return false;"><?
				?><?=$sImage?></a><?
		else:
			?><a href="<?=$arItem["URL"]?>" title="<?=htmlspecialchars($sTitle)?>"><?=$sImage?></a><?
		endif;
	endif;
				?></td></tr>
				<tr class="b">
