...
if (strlen($CSVFILE) > 0 && !in_array($CSVFILE, array('loc_ussr.csv', 'loc_usa.csv', 'loc_ussr.csv')))
{
				...
				if (strlen($strWarning)<=0)
				{
					...
					while ($arRes = $csvFile->Fetch())
					{
						...
						if ($cur_time >= $finish_time)
						{
							...
							echo "<script>Import(1, {AMOUNT:".CUtil::JSEscape($amount).",POS:".CUtil::JSEscape($cur_step)."}); sessid='".bitrix_sessid()."';</script>";
			...
			if ($LOADZIP == "Y" && file_exists($_SERVER['DOCUMENT_ROOT'].'/bitrix/wizards/bitrix/sale.locations/upload/'.DLZIPFILE))
			{
				...
				while ($arRes = $csvFile->Fetch())
				{
					...
					if ($cur_time >= $finish_time)
					{
						...
						echo "<script>Import(2, {AMOUNT:".CUtil::JSEscape($amount).",POS:".CUtil::JSEscape($cur_step)."}); sessid='".bitrix_sessid()."';</script>";
