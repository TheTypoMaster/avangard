...
class CAllSaleOrder
{
		...
		$deliveryId, $paySystemId, $arOptions, &$arErrors, &$arWarnings)
	{
		...
		$events = GetModuleEvents("sale", "OnSaleCalculateOrderShoppingCart");
		...
		$events = GetModuleEvents("sale", "OnSaleCalculateOrderPersonType");
		while ($arEvent = $events->Fetch())
			ExecuteModuleEventEx($arEvent, array(&$arOrder));
		...
		$events = GetModuleEvents("sale", "OnSaleCalculateOrderProps");
		while ($arEvent = $events->Fetch())
			ExecuteModuleEventEx($arEvent, array(&$arOrder));
		...
		$events = GetModuleEvents("sale", "OnSaleCalculateOrderDiscount");
		while ($arEvent = $events->Fetch())
			ExecuteModuleEventEx($arEvent, array(&$arOrder));
		...
		$events = GetModuleEvents("sale", "OnSaleCalculateOrderShoppingCartTax");
		while ($arEvent = $events->Fetch())
			ExecuteModuleEventEx($arEvent, array(&$arOrder));
		...
		$events = GetModuleEvents("sale", "OnSaleCalculateOrderDelivery");
		while ($arEvent = $events->Fetch())
			ExecuteModuleEventEx($arEvent, array(&$arOrder));
		...
		$events = GetModuleEvents("sale", "OnSaleCalculateOrderDeliveryTax");
		while ($arEvent = $events->Fetch())
			ExecuteModuleEventEx($arEvent, array(&$arOrder));
		...
		$events = GetModuleEvents("sale", "OnSaleCalculateOrderPaySystem");
		while ($arEvent = $events->Fetch())
			ExecuteModuleEventEx($arEvent, array(&$arOrder));
		...
		$events = GetModuleEvents("sale", "OnSaleCalculateOrder");
		while ($arEvent = $events->Fetch())
			ExecuteModuleEventEx($arEvent, array(&$arOrder));
	...
	static function DoSaveOrder($arOrder, $arAdditionalFields, $orderId, &$arErrors, $arCoupons = array())
	{
		global $APPLICATION;
		...
		CSaleBasket::DoSaveOrderBasket($orderId, $arOrder["SITE_ID"], $arOrder["USER_ID"], $arOrder["BASKET_ITEMS"], $arErrors, $arCoupons);
	...
	*/
	public function GetHistoryList($arOrder = array("ID"=>"DESC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		if (!$arSelectFields || count($arSelectFields) <= 0 || in_array("*", $arSelectFields))
			$arSelectFields = array("ID", "H_USER_ID", "H_DATE_INSERT", "H_ORDER_ID", "H_CURRENCY", "PERSON_TYPE_ID", "PAYED", "DATE_PAYED", "EMP_PAYED_ID", "CANCELED", "DATE_CANCELED", "REASON_CANCELED", "STATUS_ID", "DATE_STATUS", "PRICE_DELIVERY", "ALLOW_DELIVERY", "DATE_ALLOW_DELIVERY", "PRICE", "CURRENCY", "DISCOUNT_VALUE", "USER_ID", "PAY_SYSTEM_ID", "DELIVERY_ID", "PS_STATUS", "PS_STATUS_CODE", "PS_STATUS_DESCRIPTION", "PS_STATUS_MESSAGE", "PS_SUM", "PS_CURRENCY", "PS_RESPONSE_DATE", "TAX_VALUE", "STAT_GID", "SUM_PAID", "PAY_VOUCHER_NUM", "PAY_VOUCHER_DATE", "AFFILIATE_ID", "DELIVERY_DOC_NUM", "DELIVERY_DOC_DATE");
