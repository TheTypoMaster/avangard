...
while ($arOrder = $dbOrderList->NavNext(true, "f_"))
{
	...
	if (in_array("STATUS_ID", $arVisibleColumns))
	{
			...
			|| $row->bEditMode && !$arStatusList)
		{
			...
			if (in_array("STATUS_ID", $arVisibleColumns))
			{
				...
				$fieldValueTmp .= "[";
				if ($saleModulePermissions >= "W")
					$fieldValueTmp .= "<a href=\"/bitrix/admin/sale_status_edit.php?ID=".$arOrder["STATUS_ID"]."&lang=".LANG."\">";
				...
				if (IntVal($arOrder["EMP_STATUS_ID"]) > 0)
				{
					...
					$fieldValueTmp .= "<br />[<a href=\"/bitrix/admin/user_edit.php?ID=".$arOrder["EMP_STATUS_ID"]."&lang=".LANG."\">".$arOrder["EMP_STATUS_ID"]."</a>] ";
	...
	if (in_array("USER", $arVisibleColumns))
	{
			...
			|| !is_array($LOCAL_PAYED_USER_CACHE[$arOrder["USER_ID"]]))
		{
			...
			if ($arUser = $dbUser->Fetch())
			{
				//$LOCAL_PAYED_USER_CACHE[$arOrder["USER_ID"]] = htmlspecialcharsEx($arUser["NAME"].((strlen($arUser["NAME"])<=0 || strlen($arUser["LAST_NAME"])<=0) ? "" : " ").$arUser["LAST_NAME"])." (<a target=\"_blank\" href=\"/bitrix/admin/sale_buyers_profile.php?ID=".$arOrder["USER_ID"]."&lang=".LANG."\">".$arUser["LOGIN"]."</a>)";
				$LOCAL_PAYED_USER_CACHE[$arOrder["USER_ID"]] =
			}
		}
...
$lAdmin->sEpilogContent = "<script>if (document.getElementById('order_sum')) {setTimeout(function(){document.getElementById('order_sum').innerHTML = '".CUtil::JSEscape($order_sum)."';}, 10);}</script>";
...
$arFilterFieldsTmp = array(
	...
	"filter_cupon" => GetMessage("SO_DISCOUNT_COUPON"),
);
