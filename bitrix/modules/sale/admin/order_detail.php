...
if (isset($ORDER_AJAX) AND $ORDER_AJAX == "Y" AND check_bitrix_sessid())
{
	...
	*/
	...
	if (isset($_REQUEST["change_delivery_form"]) && $_REQUEST["change_delivery_form"] == "Y")
	{
		...
		{
			$arResultTmp = fChangeOrderStatus($ID, $_REQUEST["STATUS_ID"], false);
...
{
	$countFilterstatus = count($filter_status_id);
	for ($i = 0; $i < $countFilterstatus; $i++)
	{
			...
			$tabControl->AddSection("order_user", GetMessage("P_ORDER_USER_ACC"));
				...
				?>
					...
					$dbProperties = CSaleOrderProps::GetList(
						array(),
						array("ORDER_ID" => $ID, "PERSON_TYPE_ID" => $arPersonType["ID"], "ACTIVE" => "Y", ">INPUT_FIELD_LOCATION" => 0),
						false,
						false,
					...
					foreach ($arOrderPropsValue as $arOrderProps)
					{
						...
						<tr>
							...
							elseif ($arOrderProps["TYPE"] == "LOCATION")
							{
								...
								$polojenie = $arVal["COUNTRY_NAME"];

								if (strlen($arVal["REGION_NAME"]) > 0 && strlen($polojenie) > 0)
									$polojenie .= " - ".$arVal["REGION_NAME"];
								elseif (strlen($polojenie) <= 0)
									$polojenie = $arVal["REGION_NAME"];

								if (strlen($polojenie) > 0 && strlen($arVal["CITY_NAME"]) > 0)
									$polojenie .= " - ".$arVal["CITY_NAME"];


								echo htmlspecialcharsEx($polojenie);
							}
							else
							{
						...
						<script>
							...
							function fEditComment(el, type)
							{
								if (type == 'change')
								{
									...
									BX.ajax.post('/bitrix/admin/sale_order_detail.php', '<?=CUtil::JSEscape(bitrix_sessid_get())?>&ORDER_AJAX=Y&change=Y&comment='+el.value+'&ID=<?=$ID?>', fEditCommentResult);
			...
			$tabControl->AddSection("buyer_order", GetMessage("SOD_ORDER"));
				...
				<tr>
					<td colspan="2" valign="top">
						...
						//select product of basket to array
						...
						while ($arBasketTmp = $dbBasketTmp->GetNext())
						{
							...
							$arBasketProd[$arBasketTmp["PRODUCT_ID"]] = $arBasketTmp["PRODUCT_ID"];
						}
						...
						while ($arBasketPropsTmp = $dbBasketPropsTmp->GetNext())
							$arBasketProps[$arBasketPropsTmp["BASKET_ID"]][] = $arBasketPropsTmp;

						//select info product for basket to array
						$arBasketElement = array();
						$rsProd = CIBlockElement::GetList(array(), array("ID" => $arBasketProd), false, false, array("ID", "PREVIEW_PICTURE", "DETAIL_PICTURE", "IBLOCK_TYPE_ID", "IBLOCK_ID", "IBLOCK_SECTION_ID"));
						while ($arProd = $rsProd->Fetch())
						{
							if ($arProd["PREVIEW_PICTURE"] == "" && $arProd["DETAIL_PICTURE"] == "")
							{
								$arParent = CCatalogSku::GetProductInfo($arProd["ID"]);
								if ($arParent)
								{
									$rsProdSku = CIBlockElement::GetList(array(), array("ID" => $arParent["ID"]), false, false, array("PREVIEW_PICTURE", "DETAIL_PICTURE"));
									$arProdSku = $rsProdSku->Fetch();
									$arProd["PREVIEW_PICTURE"] = $arProdSku["PREVIEW_PICTURE"];
									$arProd["DETAIL_PICTURE"] = $arProdSku["DETAIL_PICTURE"];
								}
							}

							$arBasketElement[$arProd["ID"]] = $arProd;
						}
						...
						<tr>
							<td class="photo">
								...
								if ($bModuleInclud)
								{
									$arProductInfo = $arBasketElement[$arBasket["PRODUCT_ID"]];
							...
							<td class="balance_count">
								<?
								$balance = 0;
								if ($arBasket["MODULE"] == "catalog" && $bModuleInclud)
								{
							...
							<td class="props">
								<?
								if (is_array($arBasketProps[$arBasket["ID"]]) && count($arBasketProps[$arBasket["ID"]]) > 0)
								{
									foreach ($arBasketProps[$arBasket["ID"]] as $val)
										echo htmlspecialcharsEx($val["NAME"]).": ".htmlspecialcharsEx($val["VALUE"])."<br />";
								}
								?>
							</td>
							<td class="order_price" nowrap>
									...
									<?if ($priceDiscount > 0):?>
										...
										?>
										<div class="discount"><?=wordwrap($discountName, 40, "<br>")." ".$priceDiscount?>%</div>
