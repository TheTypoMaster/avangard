...
if (!$errorMessage) {
	...
	if (($arID = $lReports->GroupAction()) && $saleModulePermissions >= 'W')
	{
		if ($_REQUEST['action_target'] === 'on' || $_REQUEST['action_target'] === 'selected')
		{
			...
			while ($row = $res->Fetch()) $arID[] = $row['ID'];
			unset($res);
			unset($row);
		}
	...
	while ($arRes = $dbRepList->NavNext(false))
	{
		...
		$lRow->AddViewField('TITLE', "<a href='" . "sale_report_view.php?ID=" . $arRes['ID'] . "&lang=" . LANG . "' title='" . $arRes['DESCRIPTION'] . "'>" . $arRes['TITLE'] . "</a>");
		$lRow->AddViewField('CREATED_DATE', FormatDate($DB->DateFormatToPHP(CSite::GetDateFormat("SHORT")), strtotime($arRes['CREATED_DATE'])));
		...
		if ($saleModulePermissions >= 'D')
		{
			$arRowActions[] = array(
				...
				"ACTION"=>$lReports->ActionRedirect("sale_report_view.php?ID=".$arRes['ID']."&lang=".LANG),
				"DEFAULT"=>true
			);
		}
