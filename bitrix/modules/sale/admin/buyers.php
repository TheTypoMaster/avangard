...
$arHeaders = array(
	...
	array("id"=>"LID","content"=>GetMessage('BUYER_LID'), "sort"=>"LID"),
);
...
{
	$date2_stm = MkDateTime(FmtDate($filter_register_date_2,"D.M.Y"),"d.m.Y");
	if($date2_stm && strlen(trim($filter_register_date_2))>0)
		$arFilter["<DATE_REGISTER"] = trim($filter_register_date_2);
}
...
$oFilter = new CAdminFilter(
	...
	array(
		...
		"filter_lid" => GetMessage("BUYER_LID"),
	)
);
...
?>
	...
	<tr>
		<td><?echo GetMessage("BUYER_LID");?>:</td>
		<td>
			<select name="filter_lid">
				<?
				$dbSitesList = CSite::GetList($by="id", $order="asc", array("ACTIVE" => "Y"));
