...
?>
	...
	<tr>
		...
		<td width="60%">
			<?
				foreach($SALE_FIELD_TYPES as $k => $v)
					$arSaleFieldType[$k] = Array("DESCRIPTION"=>$v);
			...
			<select name="TYPE">
				...
				foreach ($arSaleFieldType as $key => $value):
					?><option value="<?echo $key?>"<?if ($str_TYPE==$key) echo " selected"?>>[<?echo htmlspecialchars($key) ?>] <?echo htmlspecialchars($value["DESCRIPTION"]) ?></option><?
				endforeach;
				?>
			</select>
		</td>
