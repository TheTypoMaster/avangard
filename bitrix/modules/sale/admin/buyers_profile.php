...
if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'viewed_apply' && IntVal($_REQUEST["viewed_id"]) > 0 && strlen($_REQUEST["viewed_lid"]) > 0 && $saleModulePermissions >= "W")
{
	...
	if (strlen($viewedError) <= 0)
	{
		...
		if (!CSaleBasket::Add($arFields))
		{
			...
			$viewedMessage = GetMessage('BUYER_VIEWED_MESS_BUSKET');
	}
}
...
if (isset($_REQUEST['apply']) && isset($_REQUEST['action']) && $saleModulePermissions >= "W" && check_bitrix_sessid())
{
	...
	if($arFuserItems = CSaleUser::GetList(array("USER_ID" => $ID)))
	{
		...
		else
		{
			...
			if (strlen($filter_busket_lid)>0)
				$arBasketActionFilter["LID"] = trim($filter_busket_lid);
		...
		foreach($arID as $key => $val)
		{
			...
			if ($LID != $key)
			{
				if (isset($_REQUEST['busket_apply']))
					$busketError = GetMEssage('BUYER_VIEWED_BUSKET_ERROR_TO_ORDER');

				break;
			}
		}

		if ($busketError == '' && $busketError == '')
		{
				...
				case "order_basket":
					if (count($arID[$LID]) > 0)
					{
						...
						while($arBasketEl = $dbBasketEl->Fetch())
						{
							...
							{
								$busketError = GetMessage('BUYER_BUSKET_ORDER_ERROR');
								break;
							}
						}

						if (strlen($busketError) <= 0)
						{
					...
					{
						if(isset($_REQUEST['busket_apply']) && isset($_REQUEST['BASKET_ID']))
							$busketError = GetMessage('BUYER_BUSKET_MESS_NULL');
					}
					break;
				case "delay_y":
				case "delay_n":
					...
					if (is_array($arID[$LID]))
					{
						foreach ($arID[$LID] as $PRODUCT_ID)
						{
							...
							if (!CSaleBasket::Update($arBasketEl["ID"], $arFields))
								$busketError = GetMessage('BUYER_BUSKET_ADD_ERROR');
...
if(!empty($arUser))
{
	...
	while ($arOrderMain = $dbOrderList->Fetch())
	{
		...
		$order = "<a href=\"sale_order_detail.php?ID=".$arOrderMain["ID"]."&lang=".LANG."\">".$arOrderMain["ID"]."</a>";
		$row->AddField("ID", $order);

		$dbBasketCount = CSaleBasket::GetList(array(), array("ORDER_ID" => $arOrderMain["ID"]), array("COUNT" => "ID"));
		$arBasketCount = $dbBasketCount->GetNext();
		...
		$row->AddField("PRODUCT", $arBasketCount["ID"]);
	...
	{
		$dateFrom = MkDateTime(FmtDate($filter_date_order_to,"D.M.Y"),"d.m.Y");

		if ($dateFrom)
			$arOrderFilter["DATE_TO"] = Trim($filter_date_order_to);
	}
	...
	if (strlen($filter_order_date_up_from)>0)
	{
		...
		if ($dateFrom)
			$arOrderFilter["DATE_INSERT_FROM"] = trim($filter_order_date_up_from);
	}
	if (strlen($filter_order_date_up_to)>0)
	{
		$dateFrom = MkDateTime(FmtDate($filter_order_date_up_to,"D.M.Y"),"d.m.Y");

		if ($dateFrom)
			$arOrderFilter["DATE_INSERT_TO"] = trim($filter_order_date_up_to);
	}
	...
	$dbOrderList = CSaleOrder::GetList(
		...
		false,
		false,
		array("ID", "LID", "STATUS_ID", "DATE_STATUS", "PAYED", "DATE_PAYED", "PRICE", "DATE_UPDATE", "DATE_INSERT", "CURRENCY")
	);
	...
	while ($arOrder = $dbOrderList->Fetch())
	{
		...
		$order = "<a href=\"sale_order_detail.php?ID=".$arOrder["ID"]."&lang=".LANG."\">".$arOrder["ID"]."</a>";
		$row->AddField("ID", $order);
		...
		$dbBasketList = CSaleBasket::GetList(array(), array("ORDER_ID" => $arOrder["ID"]));
		$orderProduct = "";
		while ($arBasketOrder = $dbBasketList->Fetch())
		{
			$orderProduct .= "<a href=\"".htmlspecialcharsbx($arBasketOrder["DETAIL_PAGE_URL"])."\">".htmlspecialcharsbx($arBasketOrder["NAME"])."</a> - ".$arBasketOrder["QUANTITY"].GetMessage('BUYERS_UNIT')."<br />";
	...
	if (!isset($_REQUEST["by"]))
		$arBusketSort = array("DATE_INSERT" => "DESC", "LID" => "ASC");
	else
		$arBusketSort[$by] = $order;

	$arBusketFilter = array("USER_ID" => $ID, "ORDER_ID" => "NULL");

	if (strlen($filter_busket_lid)>0)
		$arBusketFilter["LID"] = trim($filter_busket_lid);

	if (strlen(trim($busket_status)) > 0)
	{
		if ($busket_status == "avail")
		{
			$arBusketFilter["CAN_BUY"] = "Y";
			$arBusketFilter["DELAY"] = "N";
		}
		elseif ($busket_status == "no")
		{
			$arBusketFilter["CAN_BUY"] = "N";
			$arBusketFilter["SUBSCRIBE"] = "N";
		}
		elseif ($busket_status == "delay")
		{
			$arBusketFilter["CAN_BUY"] = "Y";
			$arBusketFilter["DELAY"] = "Y";
		}
		elseif ($busket_status == "subscribe")
		{
			$arBusketFilter["CAN_BUY"] = "N";
			$arBusketFilter["SUBSCRIBE"] = "Y";
		}
	}
	if (strlen(trim($busket_name_product)) > 0)
		$arBusketFilter["%NAME"] = $busket_name_product;

	CAdminMessage::ShowNote($busketMessage);
	CAdminMessage::ShowMessage($busketError);


	//update price
	$arCahceFuser = array();
	$arUpdateFilter = $arBusketFilter;
	...
	while ($arBasket = $dbBasketList->Fetch())
	{
		if (!in_array($arBasket["FUSER_ID"], $arCahceFuser))
		{
			$arCahceFuser[] = $arBasket["FUSER_ID"];
	...
	if (!isset($_REQUEST["by"]))
		$arViewSort = array("LID" => "ASC", "DATE_VISIT" => "DESC");
	else
		$arViewSort[$by] = $order;
	...
	while ($arViews = $dbViewsList->Fetch())
	{
		...
		$arResult = CSaleProduct::GetProductSku($ID, $arViews['LID'], $arViews["PRODUCT_ID"], $arViews["NAME"]);

		$arResult["POPUP_MESSAGE"] = array(
				"PRODUCT_ADD_TO_ORDER" => GetMEssage('BUYER_PD_ORDER'),
				"PRODUCT_ADD_TO_BUSKET" => GetMEssage('BUYER_PD_DELAY_N'),
		...
		if (count($arResult["SKU_ELEMENTS"]) > 0):
			...
			$linkBusket = "showOfferPopup(".CUtil::PhpToJsObject($arResult['SKU_ELEMENTS']).", ".CUtil::PhpToJsObject($arResult['SKU_PROPERTIES']).", 'busket', ".CUtil::PhpToJsObject($arResult["POPUP_MESSAGE"]).");";
		else:
			$linkOrder = 'BX.adminPanel.Redirect([], \'/bitrix/admin/sale_order_new.php?user_id='.$ID.'&lang='.LANG.'&LID='.$arViews["LID"].'&product['.$arViews["PRODUCT_ID"].']=1\', event);';
			$linkBusket = 'fAddToBusketViewed('.$arViews["PRODUCT_ID"].', \''.$arViews["LID"].'\')';
		endif;

		$arActions = array();
		$arActions[] = array("ICON"=>"edit", "TEXT"=>GetMessage("BUYER_PD_ORDER"), "ACTION"=>$linkOrder, "DEFAULT"=>true);
		$arActions[] = array("ICON"=>"edit", "TEXT"=>GetMessage("BUYER_PD_DELAY_N"), "ACTION"=>$linkBusket);
	...
	$aTabs = array(
		...
		array(
			"DIV" => "tab4",
			"TAB" => GetMessage("BUYER_BUSKET"),
			"ICON"=>"",
			"TITLE"=>GetMessage("BUYER_BUSKET_DESC"),
		),
		array(
			"DIV" => "tab5",
				...
				<form name="find_form3" method="GET" action="<?echo $APPLICATION->GetCurPage()?>?">
					...
					<tr>
						...
						<td>
							...
							<span><?=GetMessage('BUYER_F_PRICE_TO');?>&nbsp;<input type="text" size="7" name="filter_summa_to" maxlength="10" value="<?=htmlspecialcharsbx($filter_summa_to)?>">
						</td>
					</tr>
					<tr>
		...
		<tr>
			<td colspan="2">
				<form name="find_form4" method="GET" action="<?echo $APPLICATION->GetCurPage()?>?">
					...
					$arFilterFieldsTmp = array();
					if (count($arSites) > 1)
						$arFilterFieldsTmp[] = GetMessage('BUYER_BUSKET_F_LID');
					$arFilterFieldsTmp[] = GetMessage('BUYER_BUSKET_F_NAME');
					...
					?>
					<tr>
						<td><?=GetMessage('BUYER_BUSKET_F_STATUS')?>:</td>
						<td>
							<select name="busket_status">
								<option value=""><?=GetMessage('BUYER_BUSKET_F_STATUS_ALL')?></option>
								<option value="avail" <?=($busket_status=="avail" ? 'selected' : '')?> ><?=GetMessage('BUYER_BUSKET_F_STATUS_AVAIL')?></option>
								<option value="delay" <?=($busket_status=="delay" ? 'selected' : '')?>><?=GetMessage('BUYER_BUSKET_F_STATUS_DELAY')?></option>
								<option value="no" <?=($busket_status=="no" ? 'selected' : '')?>><?=GetMessage('BUYER_BUSKET_F_STATUS_NO')?></option>
								<option value="subscribe" <?=($busket_status=="subscribe" ? 'selected' : '')?>><?=GetMessage('BUYER_BUSKET_F_STATUS_SUB')?></option>
							</select>
							<input type="hidden" name="USER_ID" value="<?=$ID?>" >
						</td>
					</tr>
					<?
					$selectLID = "<select name=\"filter_busket_lid\">";
					$selectLID .= "<option value=\"\">".GetMessage('BUYER_VIEW_F_ALL')."</option>";
					foreach ($arSites as $arSite)
					{
						$selected = "";
						if ($arSite["ID"] == $filter_busket_lid)
							$selected = "selected";
						$selectLID .= "<option value=\"".$arSite["ID"]."\" ".$selected." >".htmlspecialcharsbx("[".$arSite["ID"]."]".$arSite["NAME"])."</option>";
					}
					$selectLID .= "</select>";
					...
					?>
					<tr>
						<td><?=GetMessage('BUYER_BUSKET_F_LID')?>:</td>
						<td>
							<?echo $selectLID?>
						</td>
					</tr>
					...
					?>
					<tr>
						<td><?=GetMessage('BUYER_BUSKET_F_NAME')?>:</td>
						<td>
							<? CUtil::DecodeUriComponent($busket_name_product);?>
							<input type="text" name="busket_name_product" size="48" value="<?=htmlspecialcharsbx($busket_name_product)?>" >
						</td>
					</tr>
					<?
					$oFilter4->Buttons(
		...
		<tr>
			<td colspan="2">
				...
				<div class="sale_popup_form" id="popup_form_sku_order" style="display:none;">
					...
					<input type="hidden" name="sku_to_busket_apply" id="sku_to_busket_apply" value="N" >
				...
				<script>
						var wind = new BX.PopupWindow('popup_sku', this, {
							...
							buttons: [
								new BX.PopupWindowButton({
									...
									events : {
										click : function() {
											if (BX('sku_to_busket_apply').value == "Y")
											{
												if (BX('viewed_id_apply').value.length > 0 && BX('viewed_lid_apply').value.length > 0)
												{
													fAddToBusketViewed(BX('viewed_id_apply').value, BX('viewed_lid_apply').value);
													wind.close();
												}
											}
						...
						function fAddToBusketViewed(product_id, lid)
						{
								...
								if (arSKU[i]["ID"] == selectedSkuId)
								{
									...
									if (arSKU[i]["CAN_BUY"] == "Y")
									{
										if (type == 'busket')
										{
											...
											BX('sku_to_busket_apply').value = "Y";
											btnText = BX.message('PRODUCT_ADD_TO_BUSKET');
										}
										else
										{
											BX('sku_to_busket_apply').value = "N";
