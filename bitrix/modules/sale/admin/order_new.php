...
if ($REQUEST_METHOD == "POST" && $save_order_data == "Y" && empty($dontsave) AND $saleModulePermissions >= "U" AND check_bitrix_sessid())
{
	...
	if ($_POST["BREAK_NAME"] == GetMessage('NEWO_BREAK_NAME'))
		$BREAK_NAME = "";
	if ($_POST["BREAK_LAST_NAME"] == GetMessage('NEWO_BREAK_LAST_NAME'))
		$BREAK_LAST_NAME = "";
	if ($_POST["BREAK_SECOND_NAME"] == GetMessage('NEWO_BREAK_SECOND_NAME'))
		$BREAK_SECOND_NAME = "";
				...
				foreach($_POST["PRODUCT"] as $key => $val)
				{
					if (!isset($val["BUSKET_ID"]) && intVal($val["BUSKET_ID"]) <= 0)
					{
						$dbBasket = CSaleBasket::GetList(
							array(),
							array("ORDER_ID" => "NULL", "PRODUCT_ID" => $val["PRODUCT_ID"], "USER_ID" => $str_USER_ID, "LID" => $LID),
							false,
							false,
							array("ID")
						);
...
foreach ($arBasketItem as $key => $val)
{
	...
	{
		$res = CIBlockElement::GetList(array(), array("ID" => $val["PRODUCT_ID"]), false, false, array('IBLOCK_ID', 'IBLOCK_SECTION_ID', 'IBLOCK_TYPE_ID'));
		if ($arCat = $res->Fetch())
		{
			if ($arCat["IBLOCK_ID"] > 0)
			{
				$arCat["IBLOCK_SECTION_ID"] = IntVal($arCat["IBLOCK_SECTION_ID"]);
				$arBasketItem[$key]["EDIT_PAGE_URL"] = "/bitrix/admin/iblock_element_edit.php?ID=".$val["PRODUCT_ID"]."&type=".$arCat["IBLOCK_TYPE_ID"]."&lang=".LANG."&IBLOCK_ID=".$arCat["IBLOCK_ID"]."&find_section_section=".$arCat["IBLOCK_SECTION_ID"];
			}
		}
	}
}
...
<script language="JavaScript">
	...
	function fRecalProduct(id, type, recommendet, recalcAll)
	{
		...
		for(var i in arProduct)
		{
			...
			var recalcCallback = "";
			var recalcOrder = "N";
			if (BX('BUSKET_' + i).value.length <= 0 || recalcAll == "Y")
			{
...
if (isset($_GET["product"]) && count($_GET["product"]) > 0 && !$bVarsFromForm)
{
				...
				for (var i = 0; i < arSKU.length; i++)
				{
					if (arSKU[i]["ID"] == selectedSkuId)
					{
						...
						if (arSKU[i]["CAN_BUY"] == "Y")
						{
							...
							'catalogXmlID' : '','productXmlID' : '','callback' : 'CatalogBasketCallback','orderCallback' : 'CatalogBasketOrderCallback','cancelCallback' : 'CatalogBasketCancelCallback','payCallback' : 'CatalogPayOrderCallback'}";
