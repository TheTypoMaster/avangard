...
if (!$bBadBlock)
{
		...
		if (count($arResult["SKU_ELEMENTS"]) > 0)
		{
			...
			foreach ($arResult["SKU_ELEMENTS"] as $val)
			{
				...
				if ($addDefault == "Y" || ($val["CAN_BUY"] == "Y" && $addDefault == "N"))
				{
					...
					$arParams['price'] =  CUtil::JSEscape($price);
					$arParams['priceFormated'] =  CUtil::JSEscape($priceFormated);
					$arParams['valutaFormat'] =  CUtil::JSEscape($val["VALUTA_FORMAT"]);
					$arParams['priceDiscount'] = CUtil::JSEscape($val["PRICE"] - $val["DISCOUNT_PRICE"]);
					$arParams['priceBase'] = CUtil::JSEscape($val["PRICE"]);
					$arParams['priceBaseFormat'] = CUtil::JSEscape($val["PRICE_FORMATED"]);
					$arParams['discountPercent'] = CUtil::JSEscape($val["DISCOUNT_PERCENT"]);
					$arParams['summaFormated'] = CUtil::JSEscape($summaFormated);
					$arParams['quantity'] = CUtil::JSEscape($quantity);
					$arParams['module'] = 'catalog';
					$arParams['currency'] = CUtil::JSEscape($val["CURRENCY"]);
					$arParams['balance'] = CUtil::JSEscape($balance);
	...
	echo "<script type=\"text/javascript\" src=\"/bitrix/js/main/core/core.js\"></script>";
