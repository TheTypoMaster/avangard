...
$permission = ((isset($arParams['PERMISSION']) && 
				...
				: CVoteChannel::GetGroupPermission($arChannel["ID"]));
...
if ($permission >= 1)
{
	...
	for ($questionIndex = 0, $questionSize = count($arQuestions); $questionIndex < $questionSize; $questionIndex++)
	{
		...
		for ($i=0,$cnt=sizeof($arAnswers[$questionID]); $i<$cnt; $i++)
		{
			$arAnswer = $arAnswers[$questionID][$i];
			$counterSum += $arAnswer["COUNTER"];
			$counterMax = max(intVal($arAnswer["COUNTER"]), $counterMax);
			if (strlen(trim($arAnswer['MESSAGE'])) == 0) unset($arAnswers[$questionID][$i]);
		}
