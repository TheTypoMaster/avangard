...
class CVote extends CAllVote
{
	...
	function CheckVotingIP($VOTE_ID, $REMOTE_ADDR, $KEEP_IP_SEC)
	{
		...
		$strSql = "
			...
			and VOTE_ID='$VOTE_ID'
			and (DATE_VOTE>=TO_DATE('".$check_date."','dd.mm.yyyy hh24:mi:ss') or '$KEEP_IP_SEC'='0')
			";
	...
	function GetNextStartDate($CHANNEL_ID)
	{
		...
		$strSql = "
			SELECT
				".$DB->DateToCharFunction("max(DATE_END+1/86400)")."	MIN_DATE_START
			FROM
				b_vote
			WHERE
	...
	function WrongDateInterval($CURRENT_VOTE_ID, $DATE_START, $DATE_END, $CHANNEL_ID)
	{
		...
		$CHANNEL_ID = intval($CHANNEL_ID);
		$st = "TO_DATE('".ConvertDateTime($DATE_START)."','dd.mm.yyyy hh24:mi:ss')";
		$en = (strlen($DATE_END)>0) ? "TO_DATE('".ConvertDateTime($DATE_END)."','dd.mm.yyyy hh24:mi:ss')" : "TO_DATE('31.12.2030 23:59:59','dd.mm.yyyy hh24:mi:ss')";
		$strSql = "
			SELECT ID 
			FROM b_vote 
			WHERE
	...
	function GetList(&$by, &$order, $arFilter=Array(), &$is_filtered)
	{
			...
			for ($i=0; $i<count($filter_keys); $i++)
			{
				...
				switch($key)
				{
					...
					case "DATE_START_1":
						...
						break;
					case "DATE_START_2":
						$arSqlSearch[] = "V.DATE_START<".$DB->CharToDateFunction($val, "SHORT")."+1";
						break;
					case "DATE_END_1":
						$arSqlSearch[] = "V.DATE_END>=".$DB->CharToDateFunction($val, "SHORT");
						break;
					case "DATE_END_2":
						$arSqlSearch[] = "V.DATE_END<".$DB->CharToDateFunction($val, "SHORT")."+1";
						break;
					case "LAMP":
						if ($val=="red")
							$arSqlSearch[] = "(V.ACTIVE<>'Y' or sysdate<V.DATE_START or sysdate>V.DATE_END)";
						elseif ($val=="green")
							$arSqlSearch[] = "(V.ACTIVE='Y' and sysdate>=V.DATE_START and sysdate<=V.DATE_END)";
						break;
					case "CHANNEL":
						$match = ($arFilter[$key."_EXACT_MATCH"]=="Y" && $match_value_set) ? "N" : "Y";
		...
		$strSql = "
			SELECT
				...
				ROUND(86400*(V.DATE_END-V.DATE_START))		PERIOD,
				count(Q.ID) QUESTIONS,
				decode(V.ACTIVE,
						'N','red',
						decode(sign(sysdate-V.DATE_START), 
							-1, 'red',
							decode(sign(sysdate-V.DATE_END), 
								1, 'red', 
								'green'									
								)
							)
		...
		if (!($USER->IsAdmin()))
		{
			$strSql = "
				SELECT
					...
					".$DB->DateToCharFunction("V.DATE_START")."	DATE_START,
					".$DB->DateToCharFunction("V.DATE_END")."	DATE_END,
					max(G.PERMISSION) as MAX_PERMISSION,
					decode(V.ACTIVE,
							'N','red',
							decode(sign(sysdate-V.DATE_START), 
								-1, 'red',
				...
				FROM
					b_vote V, 
					b_vote_channel C,
					b_vote_channel_2_group G
				WHERE
					$strSqlSearch
				and	C.ACTIVE = 'Y'
				and G.CHANNEL_ID = C.ID
				and G.GROUP_ID in ($groups)
				and	V.CHANNEL_ID = C.ID
				and V.ACTIVE = 'Y'
				and V.DATE_START<=sysdate					
				$where
				GROUP BY
					C.ID, C.TITLE, 
					V.ID, V.CHANNEL_ID, V.C_SORT, V.ACTIVE, V.COUNTER, V.TITLE, V.DESCRIPTION, V.DESCRIPTION_TYPE, V.IMAGE_ID, V.EVENT1, V.EVENT2, V.EVENT3, V.UNIQUE_TYPE, V.KEEP_IP_SEC, V.DELAY, V.DELAY_TYPE, V.TEMPLATE, V.RESULT_TEMPLATE, V.NOTIFY,
