...
function JCCalendar()
{
	...
	this.Show = function(obj, field, fieldFrom, fieldTo, bTime, serverTime, form_name, bHideTimebar)
	{
		...
		div.innerHTML =
			...
			'	<tr>'+
			'		<td class="bx-calendar-title-text" onmousedown="jsFloatDiv.StartDrag(arguments[0], document.getElementById(\'calendar_float_div\'));" id="calendar_float_title">'+this.mess["title"]+'</td><td width="0%"><a class="bx-calendar-close" href="javascript:void(0)" onclick="jsCalendar.Close();" title="'+this.mess["close"]+'"></a></td></tr>'+
			'</table>'+
			'</div>'+
		...
		if (!bHideTimebar)
		{
			...
			'	<tr>'+
			'		<td><a id="calendar_time_button" hidefocus="true" tabindex="-1" title="'+(this.bTime? this.mess["time_hide"]:this.mess["time"])+'" href="javascript:void(0)" onclick="jsCalendar.ToggleTime();" class="bx-calendar-button '+(this.bTime? 'bx-calendar-arrow-up' : 'bx-calendar-arrow-down')+'"></a></td>'+
			'	</tr>'+
			'</table>';
		}
	...
	this.GetMonthPage = function()
	{
		...
		s +=
			...
			'<tr>'+
				...
				'<td><a title="'+this.mess["month"]+'" href="javascript:void(0)" onclick="jsCalendar.MenuMonth(this); return jsCalendar.PreventDefault(arguments[0])" class="bx-calendar-button bx-calendar-arrow"></a></td>'+
				...
				'<td><a title="'+this.mess["year"]+'" href="javascript:void(0)" onclick="jsCalendar.MenuYear(this); return jsCalendar.PreventDefault(arguments[0])" class="bx-calendar-button bx-calendar-arrow"></a></td>'+
				'<td width="50%"></td>'+
