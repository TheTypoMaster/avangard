...
class CUserOptions
{
	...
	function GetOption($category, $name, $default_value=false, $user_id=false)
	{
		...
		if(
			!isset($__USER_OPTIONS_CACHE[$user_id])
			|| (
				$user_id == 0
				&& !isset($__USER_OPTIONS_CACHE[$user_id][$cache_key])
			)
		)
		{
			...
			while($res_array = $res->Fetch())
			{
				...
				if(!isset($__USER_OPTIONS_CACHE[$user_id][$row_cache_key]) || $res_array["COMMON"] <> 'Y')
					$__USER_OPTIONS_CACHE[$user_id][$row_cache_key] = $res_array["VALUE"];
			}
		}
		...
		$valKey = "~!~".$cache_key;
		if(!isset($__USER_OPTIONS_CACHE[$user_id][$valKey]))
			$__USER_OPTIONS_CACHE[$user_id][$valKey] = unserialize($__USER_OPTIONS_CACHE[$user_id][$cache_key]);

		return $__USER_OPTIONS_CACHE[$user_id][$valKey];
	}
