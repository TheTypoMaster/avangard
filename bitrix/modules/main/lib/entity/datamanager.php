...
abstract class DataManager
{
	...
	public static function checkFields(Result $result, array $data, $id = null)
	{
		...
		foreach (static::getEntity()->getFields() as $field)
		{
			if ($field instanceof ScalarField && $field->isRequired())
			{
				...
				{
					$result->addError(new FieldError($fieldName, /*Loc::*/getMessage("MAIN_ENTITY_FIELD_REQUIRED", array("#FIELD#"=>$field->getTitle())), FieldError::EMPTY_REQUIRED));
				}
			}
		}
		...
		foreach ($data as $k => $v)
		{
			...
			if (!$field->validateValue($v))
			{
				...
				$errMsgText = /*Loc::*/getMessage($errMsgCode, array(
					"#FIELD_NAME#" => $field->getName(),
					"#FIELD_TITLE#" => $field->getLangText()
				));

				$result->addError(new FieldError($field->getName(), $errMsgText, FieldError::INVALID_VALUE));
			}
		}
	}
	...
	public static function add(array $data)
	{
		...
		// check data
		static::checkFields($result, $data);
