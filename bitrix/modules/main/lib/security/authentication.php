...
class Authentication
{
	...
	public static function getUserByCookie()
	{
		...
		$cookiePrefix = Config\Option::get('main', 'cookie_name', 'BITRIX_SM');
		$cookieLogin = $request->getCookie($cookiePrefix.'_LOGIN');
		$cookieMd5Pass = $request->getCookie($cookiePrefix.'_UIDH');
	...
	public static function getUserByPassword($login, $password, $passwordIsOriginal = true)
	{
		...
		$event = new Main\Event(
			"main",
			"OnBeforeUserLogin",
			array("LOGIN" => $login, "PASSWORD" => $password, "PASSWORD_ORIGINAL" => $passwordIsOriginal)
		);
		$event->send();
		if (($eventResults = $event->getResults()) !== null)
		{
...
//					}
	...
	public static function setAuthentication(CurrentUser $user, $isPersistent = false)
	{
		...
		if (!$user->isAuthenticated())
		{
			$response->setCookie("UIDH", "", time() - 3600);
			return;
		}
		...
		$response->setCookie("LOGIN", $user->getLogin(), time()+60*60*24*30*60, '/', false, false, \Bitrix\Main\Config\Option::get("main", "auth_multisite", "N")=="Y");
		if ($isPersistent || \Bitrix\Main\Config\Option::get("main", "auth_multisite", "N") == "Y")
		{
			...
			$secure = (\Bitrix\Main\Config\Option::get("main", "use_secure_password_cookies", "N") == "Y" && $request->isHttps());

			if ($isPersistent)
				$response->setCookie("UIDH", $hash, time()+60*60*24*30*60, '/', false, $secure, \Bitrix\Main\HttpResponse::SPREAD_SITES | \Bitrix\Main\HttpResponse::SPREAD_DOMAIN);
			else
				$response->setCookie("UIDH", $hash, 0, '/', false, $secure, \Bitrix\Main\HttpResponse::SPREAD_SITES);
