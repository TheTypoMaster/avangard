...
class CurrentUser
{
	...
	public function __construct($userId = null)
	{
		if (($userId !== null) && !Type\Int::isInteger($userId))
			throw new \Bitrix\Main\ArgumentOutOfRangeException("userId");
		...
		if (!$this->isAuthenticated)
			$this->userGroups = array(2);
	}

	protected function loadUser()
	{
		if (!$this->isAuthenticated || !isset($this->userId))
			throw new \Bitrix\Main\SystemException("Can not load non-authenticated user");
	...
	{
		if (empty($groupId))
			throw new \Bitrix\Main\ArgumentNullException("groupId");
		if (!Type\Int::isInteger($groupId))
			throw new \Bitrix\Main\ArgumentTypeException("groupId", "int");
	...
	{
		if (empty($data))
			throw new \Bitrix\Main\ArgumentNullException("data");

		if (!isset($data["USER_ID"]) || !\Bitrix\Main\Type\Int::isInteger($data["USER_ID"]))
			throw new \Bitrix\Main\ArgumentOutOfRangeException("data");
