	...
	extends DbConnection
{
	...
	protected function queryInternal($sql, array $arBinds = null, $offset = 0, $limit = 0, \Bitrix\Main\Diag\SqlTrackerQuery $trackerQuery = null)
	{
		...
		$result = mysql_query($sql, $this->connectionResource);
		...
		if (!$result)
			throw new SqlException('Mysql query error', mysql_error($this->connectionResource));
	...
	{
		$this->connectInternal();
		return mysql_insert_id($this->connectionResource);
	}

	public function getAffectedRowsCount()
	{
		return mysql_affected_rows($this->getConnectionResource());
	}
	...
	public function getTableFields($tableName)
	{
		if (!array_key_exists($tableName, $this->tableColumnsCache))
		{
			...
			$this->connectInternal();
			$rs = mysql_list_fields($this->dbName, $tableName, $this->connectionResource);
			if ($rs > 0)
			{
	...
	{
		return sprintf("[%s] %s", mysql_errno($this->connectionResource), mysql_error($this->connectionResource));
	}
}
