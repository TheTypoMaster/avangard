...
<?
	foreach ($arResult["GROUPS_FORUMS"] as $key => $res):
		if ($res["TYPE"] == "GROUP"):
?>
			<optgroup label="<?=($res["DEPTH"] > 0 ? str_pad("", ($res["DEPTH"] - 1)*4, " ") : "").$res["NAME"]
				?>" class="forums level<?=$res["DEPTH"]?>"></optgroup>
<?
		else:
?>
			<option value="<?=$res["ID"]?>" <?=($arParams["TID"] == $res["ID"] ? "selected='selected'" : "")?> <?
				?>class="forums level<?=$res["DEPTH"]?>"><?=($res["DEPTH"] > 0 ? str_pad("", ($res["DEPTH"] + 1)*4, " ") : "").
					$res["NAME"]?></option>
<?
		endif;
	endforeach;
