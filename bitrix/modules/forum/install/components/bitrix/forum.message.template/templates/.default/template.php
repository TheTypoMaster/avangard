...
<table cellspacing="0" border="0" class="forum-post-table <?=($iNumber == 1 ? "forum-post-first " : "")?><?
	...
	?><?=($res["SHOW_TABLE_ID"] ? "id=\"message".$res["ID"]."\"" : "")?>>
	<tbody>
		<tr>
...
?>
					&nbsp;&nbsp;<span class="forum-action-delete"><noindex><a rel="nofollow" href="<?
						...
						?> onclick="return confirm(BX.message('cdt'));"><?=GetMessage("F_DELETE_TOPIC")?></a></noindex></span>
<?
		endif;
...
?>
			...
			if ($arParams['AJAX_POST'] == 'Y') {
				...
				?>onclick="return confirm(BX.message('cdm'));"<?
			} ?> href="<?=$res["URL"]["DELETE"]?>" <?
...
if ($arRes["USER"]["PERMISSION"] >= "Q")
{
	if (!function_exists("__forum_include_js_once_on_page_fmt"))
	{
		function __forum_include_js_once_on_page_fmt($res = null)
		{
			static $forum_include_js_once_on_page_fmt = false;
			if ($res !== null)
				$forum_include_js_once_on_page_fmt = $res;
			if ($forum_include_js_once_on_page_fmt === false)
			{
				$forum_include_js_once_on_page_fmt = true;
...
__forum_include_js_once_on_page_fmt();
}
?>