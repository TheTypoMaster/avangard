...
if (array_key_exists('NAME_LID', $arSelectFieldsMap))
{
	...
	{
		$arLangList[$arPriceLang['LID']] = $arPriceLang['NAME'];
	}
}

$dbResultList = CCatalogGroup::GetList(array($by => $order), array(), false, false, $arSelectFields);
...
while ($arRes = $dbResultList->Fetch())
{
	...
	if (array_key_exists('NAME_LID', $arSelectFieldsMap))
	{
		...
		$rsLangs = CCatalogGroup::GetLangList(array("CATALOG_GROUP_ID" => $arRes['ID']));
		...
		foreach ($arLangList as $strLangID => $strLangName)
		{
			$arLangResult[] = htmlspecialcharsex($strLangName.": ".(isset($arGroupLangList[$strLangID]) ? $arGroupLangList[$strLangID] : ''));
		}
		$row->AddViewField("NAME_LID", implode('<br>', $arLangResult));
	}
