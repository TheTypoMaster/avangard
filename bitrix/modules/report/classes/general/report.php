...
class CReport
{
	...
	public static function Add($settings)
	{
		...
		// pre-events
		$rsEvents = GetModuleEvents("report", "OnBeforeReportAdd");
		...
		// post-events
		$rsEvents = GetModuleEvents("report", "OnReportAdd");
		while ($arEvent = $rsEvents->Fetch())
		{
	...
	public static function Update($ID, $settings)
	{
		...
		// pre-events
		$rsEvents = GetModuleEvents("report", "OnBeforeReportUpdate");
		while ($arEvent = $rsEvents->Fetch())
		{
		...
		if ($result)
		{
			$rsEvents = GetModuleEvents("report", "OnReportUpdate");
			while ($arEvent = $rsEvents->Fetch())
			{
				ExecuteModuleEventEx($arEvent, array($ID, &$fields));
			}
	...
	public static function Delete($ID)
	{
		...
		// pre-events
		$rsEvents = GetModuleEvents("report", "OnBeforeReportDelete");
		while ($arEvent = $rsEvents->Fetch())
		{
		...
		// post-events
		$rsEvents = GetModuleEvents("report", "OnReportDelete");
		while ($arEvent = $rsEvents->Fetch())
		{
	...
	function generateChains($strChains, $initEntity, $initKey)
	{
		$chains = array();
			...
			if (!empty($elem['aggr']))
			{
				...
				if ($chain->hasBackReference() && $elem['aggr'] != 'GROUP_CONCAT')
				{
					...
					if ($confirm)
					{
						...
						{
							$filter['='.$primary] = new CSQLWhereExpression('?#', strtolower($entity->getCode()).'.'.$primary);
						}
	...
	public static function rewriteUserShortName(&$select, $format, $entity, $grc =false)
	{
		...
		{
			if (
				(is_string($def) && substr($def, -11) == '.SHORT_NAME')
	...
	public static function getFormattedNameExpr($format, $defPrefix)
	{
		...
		$values = array(
			'#NAME#' => array('%s', 'NAME'),
			'#NAME_SHORT#' => array($DB->Concat("UPPER(".$DB->Substr("%s", 1, 1).")", "'.'"), 'NAME'),
			'#SECOND_NAME#' => array('%s', 'SECOND_NAME'),
			'#SECOND_NAME_SHORT#' => array($DB->Concat("UPPER(".$DB->Substr("%s", 1, 1).")", "'.'"), 'SECOND_NAME'),
			'#LAST_NAME#' => array('%s', 'LAST_NAME'),
			'#LAST_NAME_SHORT#' => array($DB->Concat("UPPER(".$DB->Substr("%s", 1, 1).")", "'.'"), 'LAST_NAME')
		);
		...
		foreach ($matches as $match)
		{
			if (array_key_exists($match, $values))
			{
				...
				$sql_fields[] = $defPrefix.'.'.$values[$match][1];
			}
			elseif ($match !== '')
			{
				$expression[] = "'".$match."'";
