...
try
{
	...
	foreach ($settings['filter'] as $fId => &$fInfo)
	{
		foreach ($fInfo as $k => &$fElem)
		{
			if (is_array($fElem) && $fElem['type'] == 'field')
			{
				...
				if ($field->GetDataType() == 'datetime' && $fElem['compare'] == 'EQUAL')
				{
					// clone filter
					$fElem_start = $fElem;
					$fElem_end = $fElem;
		...
		foreach ($fInfo as $k => &$fElem)
		{
			if (is_array($fElem) && $fElem['type'] == 'field')
			{
				...
				if ($chain->hasBackReference())
				{
					...
					$_sub_init_table_alias = strtolower($entity->getCode());
	...
	// <editor-fold defaultstate="collapsed" desc="rewrite User SHORT_NAME">
	CReport::rewriteUserShortName($select, $arParams['USER_NAME_FORMAT'], $entity);

	if (substr(key($order), -11) == '.SHORT_NAME')
	{
		$_alias = Entity\QueryChain::getAliasByDefinition($entity, key($order));
		$order = array($_alias => current($order));
	}
	// </editor-fold>
	...
	call_user_func_array(
		...
		array(&$total, &$viewColumnsByResultName)
	);
}
catch (Exception $e)
{
