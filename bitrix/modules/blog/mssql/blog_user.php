...
class CBlogUser extends CAllBlogUser
{
	...
	function Add($arFields)
	{
		...
		$arInsert = $DB->PrepareInsert("b_blog_user", $arFields, "blog/avatar");
		...
		if ($ID)
		{
			if (CModule::IncludeModule("search"))
			{
				...
				$arSearchIndex = array(
					"SITE_ID" => $arPostSite,
					"LAST_MODIFIED" => ConvertTimeStamp(false, "FULL", false),//$arBlogUser["LAST_VISIT"],
					"PARAM1" => "USER",
	...
	function GetUserFriendsList($ID, $userID = False, $bAuth = False, $limit = 20)
	{
		global $DB;
		...
		$strSql =
			"SELECT TOP ".$limit." BP.ID, BP.DATE_PUBLISH, B.ID as BLOG_ID, B.URL ".
			"FROM b_blog B1 ".
		...
		$strSql .=
			...
			"	AND B.ACTIVE = 'Y' ".
			"	AND B1.ACTIVE = 'Y' ".
			"	AND (UGP.PERMS > '".$DB->ForSql(BLOG_PERMS_DENY)."' ".
