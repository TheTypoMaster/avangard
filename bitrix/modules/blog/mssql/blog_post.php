...
class CBlogPost extends CAllBlogPost
{
	...
	function GetByID($ID)
	{
		...
		else
		{
			$strSql =
				...
				"	P.ENABLE_COMMENTS, CASE WHEN P.DATE_PUBLISH <= GETDATE() THEN 'Y' ELSE 'N' END as DATE_PUBLISHED, ".
	...
	function GetList($arOrder = Array("ID" => "DESC"), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		if (count($arSelectFields) <= 0)
			$arSelectFields = array("ID", "TITLE", "BLOG_ID", "AUTHOR_ID", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_TEXT", "DETAIL_TEXT_TYPE", "DATE_CREATE", "DATE_PUBLISH", "KEYWORDS", "PUBLISH_STATUS", "CATEGORY_ID", "ATRIBUTE", "ATTACH_IMG", "ENABLE_TRACKBACK", "ENABLE_COMMENTS", "NUM_COMMENTS", "NUM_TRACKBACKS");
	...
	function GetListCalendar($blogID, $year = false, $month = false, $day = false)
	{
		...
		$strSql = 
			"SELECT CONVERT(datetime, CONVERT(varchar, P.DATE_PUBLISH, 101), 101) as DATE_PUBLISH1, COUNT(P.ID) as CNT ".
			...
			"	".$strWherePerms." ".
			"GROUP BY CONVERT(datetime, CONVERT(varchar, P.DATE_PUBLISH, 101), 101) ".
