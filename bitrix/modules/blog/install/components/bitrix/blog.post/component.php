...
if (CModule::IncludeModule("socialnetwork") && (IntVal($arParams["SOCNET_GROUP_ID"]) > 0 || IntVal($arParams["USER_ID"]) > 0))
{
	...
	if($bGroupMode)
	{
		...
		{
			ShowError(GetMessage(ToUpper($feature)."_SONET_MODULE_NOT_AVAIBLE"));
			return;
		}
	}
	else
...
if(!empty($arBlog) && $arBlog["ACTIVE"] == "Y" && ($arGroup["SITE_ID"] == SITE_ID || $bGroupMode))
{
	if($bIDbyCode)
				...
				if (CBlogPost::Delete($arParams["ID"]))
				{
					...
					if($bSoNet && $arBlog["OWNER_ID"] != $user_id)
					{
						...
						if($arB = $dbB->Fetch())
						{
							...
							BXClearCache(True, "/".SITE_ID."/blog/".$arB["URL"]."/pages/");
						}
					}
