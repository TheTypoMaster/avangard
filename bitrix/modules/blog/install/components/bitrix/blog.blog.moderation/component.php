...
if(!empty($arBlog) && $arBlog["ACTIVE"] == "Y")
{
		...
		if($bGroupMode || $arGroup["SITE_ID"] == SITE_ID)
		{
			...
			if($arResult["PostPerm"]>=BLOG_PERMS_MODERATE)
			{
				...
				if (IntVal($_GET["del_id"]) > 0)
				{
					...
					else
					{
						...
						(!$bSoNet && CBlogPost::CanUserDeletePost($DEL_ID, $user_id) || 
							($bSoNet && CBlogSoNetPost::CanUserDeletePost($DEL_ID, $user_id, $arParams["USER_ID"], $arParams["SOCNET_GROUP_ID"])))
						)
						{
						...
						if (check_bitrix_sessid())
						{
							...
							if($arResult["PostPerm"]>=BLOG_PERMS_MODERATE)
							{
								if($arPost = CBlogPost::GetByID($show_id))
								{
									...
									{
										if($bSoNet && $arPost["ID"] && CModule::IncludeModule("socialnetwork"))
										{	
											$allowHTML = COption::GetOptionString("blog","allow_html", "N");
											$allowVideo = COption::GetOptionString("blog","allow_video", "Y");
											$parserBlog = new blogTextParser(false, $arParams["PATH_TO_SMILE"]);

											if($arPost["DETAIL_TEXT_TYPE"] == "html" && $allowHTML == "Y" && $arBlog["ALLOW_HTML"] == "Y")
											...
											if (intval($arPost["AUTHOR_ID"]) > 0)
												$arSoFields["USER_ID"] = $arPost["AUTHOR_ID"];
											...
											if (intval($logID) > 0)
											{
												...
												CSocNetLogRights::SetForSonet($logID, $arSoFields["ENTITY_TYPE"], $arSoFields["ENTITY_ID"], ($arPost["MICRO"] == "Y" ? "microblog" : "blog"), "view_post", true);
												CSocNetLog::SendEvent($logID, "SONET_NEW_EVENT", $logID);
											}
										}

										if ($bGroupMode && method_exists("CSocNetGroup", "GetSite"))
										{
											$arSites = array();
										...
										foreach ($arSites as $site_id_tmp)
										{
											...
											BXClearCache(True, "/".$site_id_tmp."/blog/".$arBlog["URL"]."/post/".$show_id."/");
											...
											BXClearCache(True, "/".$site_id_tmp."/blog/".$arBlog["URL"]."/trackback/".$show_id."/");
