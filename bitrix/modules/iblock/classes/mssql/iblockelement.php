...
class CIBlockElement extends CAllIBlockElement
{
		...
		if (CModule::IncludeModule("workflow"))
		{
			...
			if ($HISTORY_DAYS>=0)
			{
				$strSql =
					...
					"WHERE TIMESTAMP_X <= dateadd(day, -".$HISTORY_DAYS.", getdate()) ".
			...
			SELECT
				...
				case
					when WF_DATE_LOCK is null then 'green'
						...
						case
							when dateadd(minute, ".$MAX_LOCK.", WF_DATE_LOCK)<getdate() then 'green'
							else
								case
	...
	function WF_GetHistoryList($ELEMENT_ID, &$by, &$order, $arFilter=Array(), &$is_filtered)
	{
		...
		if(is_array($arFilter))
		{
			...
			for ($i=0; $i<count($filter_keys); $i++)
			{
				...
				case "ID":
					...
					break;
				case "TIMESTAMP_FROM":
					$arSqlSearch[] = "E.TIMESTAMP_X >= ".$DB->CharToDateFunction($val, "SHORT");
					break;
				case "TIMESTAMP_TO":
					$arSqlSearch[] = "E.TIMESTAMP_X < dateadd(day, 1, ".$DB->CharToDateFunction($val, "SHORT").")";
					break;
				case "MODIFIED_BY":
		...
		$strSql =
			"SELECT E.*, ".
			"	E.TIMESTAMP_X, ".$DB->DateToCharFunction("E.TIMESTAMP_X")."		TIMESTAMP_X, ".
	...
	function GetList($arOrder=Array("SORT"=>"ASC"), $arFilter=Array(), $arGroupBy=false, $arNavStartParams=false, $arSelectFields=Array())
	{
		...
		*/
		...
		$arIblockElementFields = Array(
				"ID"=>"BE.ID",
				"TIMESTAMP_X"=>"BE.TIMESTAMP_X, ".$DB->DateToCharFunction("BE.TIMESTAMP_X"),
				"MODIFIED_BY"=>"BE.MODIFIED_BY",
				"DATE_CREATE"=>"BE.DATE_CREATE, ".$DB->DateToCharFunction("BE.DATE_CREATE"),
				...
				*/				/*AF_TMP for DISTINCT*/
				"ACTIVE_FROM"=>"BE.ACTIVE_FROM as AF_TMP, ".(
						...
						?
							"BE.ACTIVE_FROM, ".$DB->DateToCharFunction("BE.ACTIVE_FROM", CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "SHORT"))
						:
							"
								BE.ACTIVE_FROM, case
									when datepart(second, BE.ACTIVE_FROM)=0 then ".$DB->DateToCharFunction("BE.ACTIVE_FROM", "SHORT")."
									else ".$DB->DateToCharFunction("BE.ACTIVE_FROM", "FULL")."
								end
							"
						),
				"ACTIVE_TO"=>(
						CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "-")!="-"
						?
							"BE.ACTIVE_TO, ".$DB->DateToCharFunction("BE.ACTIVE_TO", CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "SHORT"))
						:
							"
								BE.ACTIVE_TO, case
									when datepart(second, BE.ACTIVE_TO)=0 then ".$DB->DateToCharFunction("BE.ACTIVE_TO", "SHORT")."
									else ".$DB->DateToCharFunction("BE.ACTIVE_TO", "FULL")."
								end
							"
						),
				"DATE_ACTIVE_FROM"=>(
						CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "-")!="-"
						?
							"BE.ACTIVE_FROM DATE_ACTIVE_FROM, ".$DB->DateToCharFunction("BE.ACTIVE_FROM", CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "SHORT"))
						:
							"
								BE.ACTIVE_FROM DATE_ACTIVE_FROM, case
									when datepart(second, BE.ACTIVE_FROM)=0 then ".$DB->DateToCharFunction("BE.ACTIVE_FROM", "SHORT")."
									else ".$DB->DateToCharFunction("BE.ACTIVE_FROM", "FULL")."
								end
							"
				...
				"DATE_ACTIVE_TO"=>(
						CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "-")!="-"
						?
							"BE.ACTIVE_TO DATE_ACTIVE_TO, ".$DB->DateToCharFunction("BE.ACTIVE_TO", CPageOption::GetOptionString("iblock", "FORMAT_ACTIVE_DATES", "SHORT"))
						:
							"
								BE.ACTIVE_TO DATE_ACTIVE_TO, case
									when datepart(second, BE.ACTIVE_TO)=0 then ".$DB->DateToCharFunction("BE.ACTIVE_TO", "SHORT")."
									else ".$DB->DateToCharFunction("BE.ACTIVE_TO", "FULL")."
								end
							"
...
//				"WF_LOCK"=>"BE.WF_LOCK",
				"LOCK_STATUS"=>
					"
					case
						when BE.WF_DATE_LOCK is null then 'green'
						else
							case
								when dateadd(minute, $MAX_LOCK, BE.WF_DATE_LOCK)<getdate() then 'green'
								else
									case
										when BE.WF_LOCKED_BY=$uid then 'yellow'
										else
											'red'
									end
							end
...
//				"WF_LAST_HISTORY_ID"=>"BE.WF_LAST_HISTORY_ID",
		...
		$bDistinct = false; //for mssql distinct for SECTION_ID filter not need
		...
		if($bDistinct && (!in_array("*", $arSelectFields) && !in_array("SEARCHABLE_CONTENT", $arSelectFields) && !in_array("DETAIL_TEXT", $arSelectFields)))
		{
			...
			{
				$fname = preg_replace("'^([^ ]+).*'", "\\1", trim($arSqlOrder[$i]));
				if(strpos($sSelect, $fname." ")===false)
					$sSelect .= ", ".$fname;
			}
		}
		else
		...
		{
			$res_cnt = $DB->Query("SELECT COUNT('x') AS C FROM (".
				str_replace(
					"E.TIMESTAMP_X,",
					"",
					str_replace(
						array(
							...
							"BE.ACTIVE_FROM,",
							"BE.ACTIVE_TO,",
							"BE.ACTIVE_FROM DATE_ACTIVE_FROM,",
							"BE.ACTIVE_TO DATE_ACTIVE_TO,",
						),
						"",
						$strSql
					)
				).
				") AS S", false, "FILE: ".__FILE__."<br> LINE: ".__LINE__);
		...
		else
		{
			...
			if(is_array($arNavStartParams) && IntVal($arNavStartParams["nTopCount"])>0)
				$strSql = preg_replace("#^([\s]*select[\s]+)(.+)$#is", "SELECT TOP ".$arNavStartParams['nTopCount']." \\2", $strSql);
			$res = $DB->Query($strSql, false, "FILE: ".__FILE__."<br> LINE: ".__LINE__);
			$res = new CIBlockResult($res);
			$res->arIBlockMultProps = $arIBlockMultProps;
			$res->arIBlockConvProps = $arIBlockConvProps;
		...
		else
		{
			...
			$strSql =
				"INSERT INTO b_iblock_element(".
					...
					$arInsert[0].") ".
				"VALUES(".
					(!is_set($arFields, "TIMESTAMP_X")?"getdate(), ":"").
					(!is_set($arFields, "DATE_CREATE")?"getdate(), ":"").
	...
	function Update($ID, $arFields, $bWorkFlow=false, $bUpdateSearch=true)
	{
		...
		else
		{
			...
			$strSql = "UPDATE b_iblock_element SET ".($bTimeStampNA?"TIMESTAMP_X=TIMESTAMP_X, ":"").$strUpdate." WHERE ID=".$ID;
			$DB->Query($strSql, false, "FILE: ".__FILE__."<br> LINE: ".__LINE__);
		...
		foreach($ar_prop as $prop)
		{
			...
			if($prop["PROPERTY_TYPE"]=="L")
			{
				...
				if($prop["VERSION"]==2 && $prop["MULTIPLE"]=="N")
				{
					$strSql = "
						...
						WHERE
							b_iblock_element_prop_s".$prop["IBLOCK_ID"].".IBLOCK_ELEMENT_ID=".$ELEMENT_ID."
							AND P.ID=".$prop["ID"]."
			...
			else
			{
				...
				if($cacheValues===false || $prop["VERSION"]==2)
				{
					...
					{
						$strSql = "
							SELECT	CAST(IBLOCK_ELEMENT_ID AS VARCHAR)+':'+'".$prop["ID"]."' ID, PROPERTY_".$prop["ID"]." VALUE
	...
	{
		return " isnull(BE.SHOW_COUNTER/(datediff(second,BE.SHOW_COUNTER_START,getdate())+0.0000001),0) ";
	}
}
