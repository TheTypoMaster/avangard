...
class CIBlockProperty extends CAllIBlockProperty
{
	...
	{	
		...
		{//MULTIPLE changed
			...
			{//MULTIPLE=Y
				...
				if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR01"));
					return false;
				}
				$strSql = "
				...
				if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR02"));
					return false;
				}
				$strSql = "
					ALTER TABLE b_iblock_element_prop_s".$arProperty["IBLOCK_ID"]."
				...
				if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR03"));
					return false;
				}
			}
			else
			{//MULTIPLE=N
				...
				";
			 	if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR04"));
					return false;
				}
				$strSql = "
					ALTER TABLE b_iblock_element_prop_s".$arProperty["IBLOCK_ID"]."
				...
				";
			 	if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR05"));
					return false;
				}
				switch($arFields["PROPERTY_TYPE"])
				{
				...
				if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR06"));
					return false;
				}
				$strSql = "
					DELETE FROM
				...
				if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR07"));
					return false;
				}
			}
		}
		...
		{//MULTIPLE not changed
			...
			&& $arProperty["MULTIPLE"]=="N")
			{
				...
				if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR08"));
					return false;
				}
				$strSql = "
				...
				";
			 	if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR09"));
					return false;
				}
				$strSql = "
					ALTER TABLE b_iblock_element_prop_s".$arProperty["IBLOCK_ID"]."
				...
				";
			 	if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR10"));
					return false;
				}
				switch($arFields["PROPERTY_TYPE"])
				{
				...
				if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR11"));
					return false;
				}
				$strSql = "
					DELETE FROM
				...
				if(!$DB->Query($strSql))
				{
					$this->LAST_ERROR = GetMessage("IBLOCK_PROPERTY_CHANGE_ERROR",array("#ID#"=>$ID,"#CODE#"=>"OR12"));
					return false;
				}
			}
		}
	...
	function _Add($ID, $arFields)
	{
		...
		$strSql = "
			...
			ADD (PROPERTY_".$ID." ".$strType.", DESCRIPTION_".$ID." VARCHAR2(255))
		";
