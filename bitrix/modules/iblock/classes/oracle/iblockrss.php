...
class CIBlockRSS extends CAllIBlockRSS
{
	function GetCache($cacheKey)
	{
		...
		$db_res = $DB->Query(
			"SELECT CACHE, DECODE(SIGN(CACHE_DATE-SYSDATE), 1, 'Y', 'N') as VALID ".
			"FROM b_iblock_cache ".
	...
	function Add($IBLOCK_ID, $NODE, $NODE_VALUE)
	{
		...
		$RSS_ID = IntVal($DB->NextID("SQ_IBLOCK_RSS"));
		$DB->Query(
			"INSERT INTO b_iblock_rss (ID, IBLOCK_ID, NODE, NODE_VALUE) ".
			"VALUES(".$RSS_ID.", ".$IBLOCK_ID.", '".$DB->ForSql($NODE, 50)."', '".$DB->ForSql($NODE_VALUE, 255)."')");
	}
	...
	function UpdateCache($cacheKey, $CACHE, $HOURS_CACHE, $bCACHED)
	{
		...
		if ($bCACHED)
		{
			$strSql = 
				...
				"	CACHE = EMPTY_CLOB(), ".
				"	CACHE_DATE = SYSDATE + 1/24*".IntVal($HOURS_CACHE)." ".
				"WHERE CACHE_KEY = '".$DB->ForSql($cacheKey, 0)."' ";
			$arBinds = Array();
			$arBinds["CACHE"] = $CACHE;
		...
		else
		{
			$strSql = 
				...
				"VALUES('".$DB->ForSql($cacheKey, 50)."', EMPTY_CLOB(), SYSDATE + 1/24*".IntVal($HOURS_CACHE).") ";
			$arBinds = Array();
			$arBinds["CACHE"] = $CACHE;
			$DB->QueryBind($strSql, $arBinds);
		}
	...
	function GetRSSText($arIBLOCK, $LIMIT_NUM = false, $LIMIT_DAY = false, $yandex = false)
	{
		...
		$strSql = 
			"SELECT BE.*, ".$DB->DateToCharFunction("BE.TIMESTAMP_X")." as TIMESTAMP_X, ".
			...
			"FROM b_iblock_element BE, b_iblock B, b_lang L ".
	...
	function PreGenerateRSS($IBLOCK_ID, $yandex = true)
	{
		...
		$strSql = 
			"SELECT B.*, S.CHARSET, S.SERVER_NAME, ".$DB->DateToCharFunction("B.TIMESTAMP_X")." as TIMESTAMP_X ".
			"FROM b_iblock B LEFT JOIN b_lang S ON S.LID=B.LID ".
			"WHERE ID = ".IntVal($IBLOCK_ID)." ".
			"	AND EXISTS( ".
			"		SELECT 'x' ".
			"		FROM b_iblock_group IBG ".
			"		WHERE IBG.IBLOCK_ID=B.ID ".
