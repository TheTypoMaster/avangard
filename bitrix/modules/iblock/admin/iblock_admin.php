...
while($dbrs = $rsIBlocks->NavNext(true, "f_"))
{
	...
	)
	{
		$arActions[] = array(
			...
			"ACTION"=>"if(confirm('".GetMessage("IBLOCK_ADM_CONFIRM_DEL_MESSAGE")."')) ".$lAdmin->ActionDoGroup($f_ID, "delete", "&type=".htmlspecialchars($type)."&lang=".LANG."&admin=".($_REQUEST["admin"]=="Y"? "Y": "N")),
		);
	}
