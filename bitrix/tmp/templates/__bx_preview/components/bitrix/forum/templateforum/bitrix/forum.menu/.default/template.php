...
{
	foreach ($arResult["FORUMS"] as $arGroup):
...
?><table cellspacing="0" class="forum-menu"><tr><td class="forumtoolbar">
	...
	if ($arResult["SHOW_SEARCH"] == "Y"):
			...
			<td><a href="<?=$arResult["search"]?>" title="<?=GetMessage("FMI_FORUM_SEARCH_TITLE")?>"><?=GetMessage("FMI_FORUM_SEARCH")?></a></td><?
	endif;
	...
	if ($arResult["IsAuthorized"] == "Y"):
		...
		if ($arParams["TMPLT_SHOW_AUTH_FORM"] == "INPUT"):
					?><td><a href="<?=$arResult["AUTH"]["LOGIN"]?>" title="<?=GetMessage("FMI_AUTHORIZE_TITLE")?>"><?=GetMessage("FMI_AUTHORIZE")?>:</a>&nbsp;</td>
					<td nowrap="nowrap">
						<form action="<?=POST_FORM_ACTION_URI?>" method="post" name="FORUM_AUTH_<?=$arResult["IndexForForm"]?>" id="FORUM_AUTH_<?=$arResult["IndexForForm"]?>">
					...
					?><td><a href="<?=$arResult["AUTH"]["LOGIN"]?>" title="<?=GetMessage("FMI_AUTHORIZE")?>"><?=GetMessage("FMI_ENTER")?></a></td><?
		endif;
		...
		if($arResult["new_user_registration"] == "Y"):
					...
					<td><a href="<?=$arResult["AUTH"]["REGISTER"]?>" title="<?=GetMessage("FMI_REGISTER_TITLE")?>"><?=GetMessage("FMI_REGISTER")?></a></td><?
		endif;
