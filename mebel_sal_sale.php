...
<?
if($id=='all' || $all_models) { echo "Во всех салонах Москвы и Московской области";} else {$res = CIBlockElement::GetByID($s_val);if($ar_res = $res->GetNext())  echo '<a style=" font-size: 14px; color: #000000;" href="/redesign/where_buy/detail.php?id='.$s_val.'">'.$ar_res['NAME'].'</a>';}?>
</h3>
...
<?if($all_models) { ?>
<td><a href="/catalog/divan<?=$all_models?>.htm?back=y"><?$res = CIBlockElement::GetByID(intVal($all_models));if($ar_res = $res->GetNext())  echo $ar_res['NAME'];?></a></td>
...
$arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PROPERTY_salon", "PROPERTY_razmeri", "PROPERTY_tovar","PROPERTY_color", "PROPERTY_price_old", "PROPERTY_price_new", "PROPERTY_mechanizm",  "PROPERTY_spal", "PROPERTY_aktia", "PROPERTY_sold");
...
opacity: 0.5; /* CSS3 - Mozilla 1.7b +, Firefox 0.9 +, Safari 1.2+, Opera 9 */ " alt="Товар продан" src="/images/sold.gif">';
	...
	$html_temp .='" border="0" width="200" height="100"></a><div class="highslide-caption">'.$arFields['PREVIEW_TEXT'].'</div></td></tr>
		...
		$html_temp .='</td></tr>
			...
			if($all_models) $html_temp .='<a href="/redesign/where_buy/detail.php?id='.$arFields["PROPERTY_SALON_VALUE"].'">Салон</a>';
