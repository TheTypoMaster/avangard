...
$APPLICATION->set_cookie("basket[0][1827] ", "");*/
//$APPLICATION->set_cookie("basket[0]", "");
//$APPLICATION->set_cookie("basket[0][1827]", "");
//$APPLICATION->set_cookie("basket[1]", "");
//echo "<pre>basket - ";print_r($APPLICATION->get_cookie("basket"));echo "</pre>";
...
//				echo "$key_in_basket srcTexture12 - ";				print_r($srcTexture);
		...
		<tr id="tr_<?=$key_in_basket?>">
			...
			<td><?=$srcSubjBIG?><br><?=$srcSubj?> <? if (!empty($srcSubj) && empty($srcCombinac)) { ?><a href="<?=$srcSubjBIG?>" rel="shadowbox[dop<?=$arItem["ID"]?>]"><img class="detail_picture" src="<?=$srcSubj?>"  /></a> <? } ?></td>
